...
// Mon Jul 13 21:43:01 2015 by ROOT version 6.02/05
// from TTree oniaTree/Tree of Onia2MuMu
// found on file: Rootuple_MuOniaAll.root
//////////////////////////////////////////////////////////
...
// Fixed size dimensions of array or collections stored in the TTree if any.
   ...
   SinglePD(TTree *tree=0);
   virtual ~SinglePD();
   virtual Int_t    Cut(Long64_t entry);
   virtual Int_t    GetEntry(Long64_t entry);
   virtual Long64_t LoadTree(Long64_t entry);
   virtual void     Init(TTree *tree);
   virtual void     Loop(TString PD);
...
#ifdef SinglePD_cxx
SinglePD::SinglePD(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("Rootuple_MuOniaAll.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("Rootuple_MuOniaAll.root");
      }
      TDirectory * dir = (TDirectory*)f->Get("Rootuple_MuOniaAll.root:/rootuple");
...
Int_t SinglePD::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t SinglePD::LoadTree(Long64_t entry)
{
...
void SinglePD::Show(Long64_t entry)
{
...
Int_t SinglePD::Cut(Long64_t entry)
{
